...
def fetch_and_process_players(season='2023-24'):
    """Fetch and process all active players in a single pass"""
    print(f"\n=== Fetching active players for {season} ===")
    ...
    try:
        ...
        print("First few players:", players['DISPLAY_FIRST_LAST'].head().tolist())
        print("Columns in players data:", players.columns.tolist())
        
        all_player_data = []
        batch_size = 10  # Reduced batch size to be safer
        
        # Process players in batches
        for i in tqdm(range(0, len(players), batch_size), desc="Processing players"):
            ...
            for _, player in batch.iterrows():
                ...
                try:
                    ...
                    # Get player logs with a single attempt
                    logs, success = get_player_game_logs(player_id, season)
                    
                    if not success or logs.empty:
                        print(f"  ❌ Failed to get logs for {player_name}")
                        continue
                    
                    print(f"  ✅ Got {len(logs)} games for {player_name}")
                    ...
                    if not processed_logs.empty:
                        ...
                        print("  ❌ No valid games after processing")
                        
                except Exception as e:
                    print(f"  ❌ Error processing {player_name}: {str(e)}")
                    continue
            
            # Add batch data to all_player_data
            if batch_data:
                all_player_data.extend(batch_data)
                print(f"✅ Added batch {i//batch_size + 1} with {len(batch_data)} players")
            
            # Add delay between batches
            if i + batch_size < len(players):
                time.sleep(2)  # Increased delay to 2 seconds
        
        print(f"\n✅ Processed {len(all_player_data)} players in total")
        return all_player_data
    
    except Exception as e:
        print(f"❌ Error in fetch_and_process_players: {str(e)}")
        import traceback
        traceback.print_exc()
        return []

def main():
    print("=== Starting NBA Player Stats EMA Calculation (Single Pass) ===\n")
    start_time = time.time()
    
    # Output file
    output_file = 'nba_player_rolling_stats_single_pass.csv'
    ...
    try:
        ...
        all_player_data = fetch_and_process_players('2023-24')
